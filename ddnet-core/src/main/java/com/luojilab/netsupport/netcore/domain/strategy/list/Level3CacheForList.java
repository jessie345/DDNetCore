 ...
 */
public class Level3CacheForList<T> extends ListGetStrategy<T> {
    private MemoryStorage mMemoryDataPool;

    public Level3CacheForList(@NonNull Class<T> dataClass, @NonNull Call<JsonObject> call, @Nullable ResponseListener<List<T>> callback) {
        super(dataClass, call, callback);
    ...
    private void notifyCacheSuccess(@NonNull List<T> data, boolean isDone) {
        if (mResponseListener != null) {
