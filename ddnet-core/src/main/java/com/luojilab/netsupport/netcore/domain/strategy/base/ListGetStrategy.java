 ...
 */
public abstract class ListGetStrategy<T> extends RetrofitStrategy {
    protected ResponseListener<List<T>> mResponseListener;
    protected Class<T> mDataClass;

    public ListGetStrategy(@Nullable Class<T> dataClass, @NonNull Call<JsonObject> call, @Nullable ResponseListener<List<T>> listener) {
        super(call);
