...
public class CoreUtils {
    ...
    @NonNull
    public static Pair<String, String[]> buildSqlInSegment(@NonNull Object[] ids) {
        checkNotNull(ids, "buildSqlInSegment() 使用方式错误");
    ...
    public static ContentValues json2ContentValues(@NonNull JsonObject json) {
        ...
        while (itr.hasNext()) {
            ...
            if (value == null) {
                NetLogger.d(NetLogger.TAG, "value 值为null,key:" + key);
                continue;
            }
            ...
            if (value instanceof String) {
                ...
                NetLogger.d(NetLogger.TAG, "未知的value类型,key:" + key + ",value:" + value);
    ...
    /**
     * @param cursor
     * @return 调用者不关注cursor是否转换成功，而关心转换的结果，
     * cursor无效，则结果为空list
     */
    @NonNull
    public static List<JsonObject> parseCursorToJsonList(@NonNull Cursor cursor) {
        Preconditions.checkNotNull(cursor);

        if (cursor.isClosed()) return Collections.emptyList();

        List<JsonObject> list = new ArrayList<>();

        try {
            while (cursor.moveToNext()) {
                int index = cursor.getColumnIndex(TableDef.TableNosql.Column.COLUMN_CONTENT);
                String content = cursor.getString(index);

                if (TextUtils.isEmpty(content)) continue;

                JsonObject jsonObject = CoreUtils.parseJsonObject(content);
    ...
    public static Map<String, String> json2UrlEncodedMap(@NonNull JsonObject data) {
        ...
        for (Map.Entry<String, JsonElement> entry : set) {
            ...
            if (TextUtils.isEmpty(key)) {
                NetLogger.e("key is empty,value:" + data);
                continue;
            }

            JsonElement value = entry.getValue();
            if (value == null) {
                NetLogger.e("value is empty,value:" + value);
                continue;
            }
